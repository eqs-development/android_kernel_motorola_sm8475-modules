...
static bool g_fsm_inited;

static int fsm_init_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = g_fsm_inited;

	return 0;
}

static int fsm_init_put(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	int state = ucontrol->value.integer.value[0];

	pr_info("init switch: %d", state);
	if (g_fsm_inited)
		return 0;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (stop) {
		cfg->force_mute = true;
		fsm_speaker_off();
		fsm_monitor_switch(0, false);
	} else {
		cfg->force_mute = false;
		fsm_monitor_switch(cfg->next_scene, true);
		fsm_speaker_onn();
	}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	fsm_mutex_lock();
	fsm_afe_handle_direction(false, &angle);
	fsm_mutex_unlock();
	ucontrol->value.integer.value[0] = angle;
#else
	fsm_config_t *cfg = fsm_get_config();
...
#ifdef CONFIG_FSM_CROSSFADE
	fsm_mutex_lock();
	fsm_afe_handle_direction(true, &angle);
	fsm_mutex_unlock();
#else
