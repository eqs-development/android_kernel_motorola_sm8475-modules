...
int fsm_reg_dump(fsm_dev_t *fsm_dev)
{
	...
	reg_end = (fsm_dev->is19xx ? 0xEF : (fsm_dev->is1820 ? 0xEA : 0xE8));
...
static int fsm_check_dev_type(fsm_dev_t *fsm_dev)
{
	...
	if (fsm_dev->is19xx) {
		if (HIGH8(dev_type) != FS1958_DEV_ID && HIGH8(dev_type) != FS19XX_DEV_ID) {
			pr_addr(err, "invalid type:%04X", dev_type);
			return -EINVAL;
		}
		return 0;
	...
	if (HIGH8(fsm_dev->version) == FS18HS_DEV_ID && HIGH8(dev_type) == 0x31)
		return 0;
...
int fsm_check_spkcoef(fsm_dev_t *fsm_dev)
{
	...
	spkcoef = (fsm_dev->is19xx ? 0x14 : 0x08);
	ret = fsm_reg_read(fsm_dev, spkcoef, &value);
	if (value == 0x0000 || (spkcoef == 0x08 && value == 0x0010)) {
		// not inited yet
		return -EINVAL;
	}
...
int fsm_config_vol(fsm_dev_t *fsm_dev)
{
	...
	return fsm_set_bf(fsm_dev, volctrl, volume);
}
...
void fsm_set_scene(int scene)
{
	...
	if (cfg->next_scene != BIT(scene) && cfg->speaker_on) {
